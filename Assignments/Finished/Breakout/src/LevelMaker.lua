...
function LevelMaker.createMap(level)
    ...
    if #bricks == 0 then
        ...
        -- randomly add a locked brick
        if math.random(3) > 1 then
            amoutToLock = math.floor(math.random(#bricks/5))

            while amoutToLock > 0  do
                selectedBrick = bricks[math.random(#bricks)]
                if not selectedBrick.locked then
                    selectedBrick.locked = true
                    selectedBrick.wasLocked = true
                    selectedBrick.tier = 0
                    selectedBrick.color = 1
                    amoutToLock = amoutToLock-1
                end
            end
