...
-- Init moved to first instanciation else the variable IMAGES_FOLDER is not set
local PIPE_IMAGE = false
...
function Pipe:init(orientation, y)
    ...
    if not PIPE_IMAGE then
        PIPE_IMAGE = love.graphics.newImage(IMAGES_FOLDER..'pipe.png')
    end
end
